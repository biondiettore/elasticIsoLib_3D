...
void BornElasticShotsGpu_3D::forward(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float4DReg> data) const{
	if (!add) data->zero();
    ...
    std::vector<std::shared_ptr<float4DReg>> modelSlicesVector;
    ...
    std::shared_ptr<float3DReg> temp_stag(new float3DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2), _elasticParam->getHyper()->getAxis(3)));
    std::shared_ptr<float3DReg> temp_stag1(new float3DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2), _elasticParam->getHyper()->getAxis(3)));
    std::shared_ptr<SEP::float4DReg> modelSlice(new SEP::float4DReg(hyperModelSlice));
    ...
    //drho_x
    std::memcpy(temp_stag->getVals(), model->getVals(), nModel*sizeof(float));
    staggerXop->adjoint(false, temp_stag1, temp_stag);
    std::memcpy(modelSlice->getVals(), temp_stag1->getVals(), nModel*sizeof(float));

    //drho_y
    staggerYop->adjoint(false, temp_stag1, temp_stag);
    std::memcpy(modelSlice->getVals()+nModel, temp_stag1->getVals(), nModel*sizeof(float));

		//drho_z
    staggerZop->adjoint(false, temp_stag1, temp_stag);
    std::memcpy(modelSlice->getVals()+2*nModel, temp_stag1->getVals(), nModel*sizeof(float));

    //dlame
    std::memcpy(modelSlice->getVals()+3*nModel, model->getVals()+nModel, nModel*sizeof(float) );

    //Shear modulus perturbations
    //dmu
    std::memcpy(modelSlice->getVals()+4*nModel, model->getVals()+2*nModel, nModel*sizeof(float) );
    //dmu_xz
    std::memcpy(temp_stag->getVals(), model->getVals()+2*nModel, nModel*sizeof(float) );
    staggerXop->adjoint(false, temp_stag1, temp_stag);
    staggerZop->adjoint(false, temp_stag, temp_stag1);
    std::memcpy(modelSlice->getVals()+5*nModel, temp_stag->getVals(), nModel*sizeof(float) );
		//dmu_xy
    std::memcpy(temp_stag->getVals(), model->getVals()+2*nModel, nModel*sizeof(float) );
    staggerXop->adjoint(false, temp_stag1, temp_stag);
    staggerYop->adjoint(false, temp_stag, temp_stag1);
    std::memcpy(modelSlice->getVals()+6*nModel, temp_stag->getVals(), nModel*sizeof(float) );
		//dmu_yz
    std::memcpy(temp_stag->getVals(), model->getVals()+2*nModel, nModel*sizeof(float) );
    staggerYop->adjoint(false, temp_stag1, temp_stag);
    staggerZop->adjoint(false, temp_stag, temp_stag1);
    std::memcpy(modelSlice->getVals()+7*nModel, temp_stag->getVals(), nModel*sizeof(float) );
...
void BornElasticShotsGpu_3D::adjoint(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float4DReg> data) const{
	if (!add) model->zero();
    ...
    std::shared_ptr<float3DReg> temp_stag(new float3DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2), _elasticParam->getHyper()->getAxis(3)));
    std::shared_ptr<float3DReg> temp_stag1(new float3DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2), _elasticParam->getHyper()->getAxis(3)));
		std::shared_ptr<float3DReg> temp_stag2(new float3DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2), _elasticParam->getHyper()->getAxis(3)));
    ...
    //drho_x
    std::memcpy(temp_stag->getVals(), model->getVals(), nModel*sizeof(float));
		std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals(), nModel*sizeof(float) );
    staggerXop->forward(true, temp_stag1, temp_stag);

    //drho_y
		std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals()+nModel, nModel*sizeof(float) );
		staggerYop->forward(true, temp_stag1, temp_stag);

		//drho_z
		std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals()+2*nModel, nModel*sizeof(float) );
    staggerZop->forward(true, temp_stag1, temp_stag);
		std::memcpy(model->getVals(), temp_stag->getVals(), nModel*sizeof(float) );

    //dmu_xz
    std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals()+5*nModel, nModel*sizeof(float) );
    staggerZop->forward(false, temp_stag1, temp_stag);
    staggerXop->forward(false, temp_stag, temp_stag2);
		//dmu_xy
    std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals()+6*nModel, nModel*sizeof(float) );
    staggerYop->forward(false, temp_stag1, temp_stag);
    staggerXop->forward(true, temp_stag, temp_stag2);
		//dmu_yz
    std::memcpy(temp_stag1->getVals(), modelSlicesVector[0]->getVals()+7*nModel, nModel*sizeof(float) );
    staggerZop->forward(false, temp_stag1, temp_stag);
    ...
    //drho_x
    std::memcpy(temp_stag->getVals(), model->getVals(), nModel*sizeof(float));
		std::memcpy(temp_stag1->getVals(), modelSlice->getVals(), nModel*sizeof(float) );
    staggerXop->forward(true, temp_stag1, temp_stag);

    //drho_y
		std::memcpy(temp_stag1->getVals(), modelSlice->getVals()+nModel, nModel*sizeof(float) );
		staggerYop->forward(true, temp_stag1, temp_stag);

		//drho_z
		std::memcpy(temp_stag1->getVals(), modelSlice->getVals()+2*nModel, nModel*sizeof(float) );
    staggerZop->forward(true, temp_stag1, temp_stag);
		std::memcpy(model->getVals(), temp_stag->getVals(), nModel*sizeof(float) );

    //dmu_xz
    std::memcpy(temp_stag1->getVals(), modelSlice->getVals()+5*nModel, nModel*sizeof(float) );
    staggerZop->forward(false, temp_stag1, temp_stag);
    staggerXop->forward(false, temp_stag, temp_stag2);
		//dmu_xy
    std::memcpy(temp_stag1->getVals(), modelSlice->getVals()+6*nModel, nModel*sizeof(float) );
    staggerYop->forward(false, temp_stag1, temp_stag);
    staggerXop->forward(true, temp_stag, temp_stag2);
		//dmu_yz
    std::memcpy(temp_stag1->getVals(), modelSlice->getVals()+7*nModel, nModel*sizeof(float) );
    staggerZop->forward(false, temp_stag1, temp_stag);
    staggerYop->forward(true, temp_stag, temp_stag2);
