...
void initNonlinearElasticGpu_3D(float dz, float dx, float dy, int nz, int nx, int ny, int nts, float dts, int sub, int minPad, int blockSize, float alphaCos, int nGpu, int iGpuId, int iGpuAlloc){
		...
		if (iGpuId == iGpuAlloc) {
				...
				// model (Source)
				dev_modelRegDts_vx = new float*[nGpu];
				dev_modelRegDts_vy = new float*[nGpu];
				dev_modelRegDts_vz = new float*[nGpu];
				dev_modelRegDts_sigmaxx = new float*[nGpu];
				dev_modelRegDts_sigmayy = new float*[nGpu];
				dev_modelRegDts_sigmazz = new float*[nGpu];
				dev_modelRegDts_sigmaxz = new float*[nGpu];
				dev_modelRegDts_sigmaxy = new float*[nGpu];
				dev_modelRegDts_sigmayz = new float*[nGpu];
				// data
...
void allocateNonlinearElasticGpu_3D(float *rhoxDtw, float *rhoyDtw, float *rhozDtw, float *lamb2MuDtw, float *lambDtw, float *muxzDtw, float *muxyDtw, float *muyzDtw, int nx, int ny, int nz, int iGpu, int iGpuId){
...
void modelAllocateGpu_3D(long long nSourcesRegCenterGrid, long long nSourcesRegXGrid, long long nSourcesRegYGrid, long long nSourcesRegZGrid, long long nSourcesRegXZGrid, long long nSourcesRegXYGrid, long long nSourcesRegYZGrid, int iGpu){
		// fx
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_vx[iGpu], nSourcesRegXGrid*host_nts*sizeof(float)));
		// fy
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_vy[iGpu], nSourcesRegYGrid*host_nts*sizeof(float)));
		// fz
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_vz[iGpu], nSourcesRegZGrid*host_nts*sizeof(float)));
		// mxx
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmaxx[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// myy
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmayy[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// mzz
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmazz[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// mxz
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmaxz[iGpu], nSourcesRegXZGrid*host_nts*sizeof(float)));
		// mxy
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmaxy[iGpu], nSourcesRegXYGrid*host_nts*sizeof(float)));
		// myz
		cuda_call(cudaMalloc((void**) &dev_modelRegDts_sigmayz[iGpu], nSourcesRegYZGrid*host_nts*sizeof(float)));
}
//copy source-model signals from host to device
void modelCopyToGpu_3D(float *modelRegDts_vx, float *modelRegDts_vy, float *modelRegDts_vz, float *modelRegDts_sigmaxx, float *modelRegDts_sigmayy, float *modelRegDts_sigmazz, float *modelRegDts_sigmaxz, float *modelRegDts_sigmaxy, float *modelRegDts_sigmayz, long long nSourcesRegCenterGrid, long long nSourcesRegXGrid, long long nSourcesRegYGrid, long long nSourcesRegZGrid, long long nSourcesRegXZGrid, long long nSourcesRegXYGrid, long long nSourcesRegYZGrid, int iGpu){
		// fx
		cuda_call(cudaMemcpy(dev_modelRegDts_vx[iGpu], modelRegDts_vx, nSourcesRegXGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// fy
		cuda_call(cudaMemcpy(dev_modelRegDts_vy[iGpu], modelRegDts_vy, nSourcesRegYGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// fz
		cuda_call(cudaMemcpy(dev_modelRegDts_vz[iGpu], modelRegDts_vz, nSourcesRegZGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// mxx
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmaxx[iGpu], modelRegDts_sigmaxx, nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// myy
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmayy[iGpu], modelRegDts_sigmayy, nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// mzz
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmazz[iGpu], modelRegDts_sigmazz, nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// mxz
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmaxz[iGpu], modelRegDts_sigmaxz, nSourcesRegXZGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// mxy
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmaxy[iGpu], modelRegDts_sigmaxy, nSourcesRegXYGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
		// myz
		cuda_call(cudaMemcpy(dev_modelRegDts_sigmayz[iGpu], modelRegDts_sigmayz, nSourcesRegYZGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice));
}
...
void modelInitializeOnGpu_3D(long long nSourcesRegCenterGrid, long long nSourcesRegXGrid, long long nSourcesRegYGrid, long long nSourcesRegZGrid, long long nSourcesRegXZGrid, long long nSourcesRegXYGrid, long long nSourcesRegYZGrid, int iGpu){
		// fx
		cuda_call(cudaMemset(dev_modelRegDts_vx[iGpu], 0, nSourcesRegXGrid*host_nts*sizeof(float)));
		// fy
		cuda_call(cudaMemset(dev_modelRegDts_vy[iGpu], 0, nSourcesRegYGrid*host_nts*sizeof(float)));
		// fz
		cuda_call(cudaMemset(dev_modelRegDts_vz[iGpu], 0, nSourcesRegZGrid*host_nts*sizeof(float)));
		// mxx
		cuda_call(cudaMemset(dev_modelRegDts_sigmaxx[iGpu], 0, nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// myy
		cuda_call(cudaMemset(dev_modelRegDts_sigmayy[iGpu], 0, nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// mzz
		cuda_call(cudaMemset(dev_modelRegDts_sigmazz[iGpu], 0, nSourcesRegCenterGrid*host_nts*sizeof(float)));
		// mxz
		cuda_call(cudaMemset(dev_modelRegDts_sigmaxz[iGpu], 0, nSourcesRegXZGrid*host_nts*sizeof(float)));
		// mxy
		cuda_call(cudaMemset(dev_modelRegDts_sigmaxy[iGpu], 0, nSourcesRegXYGrid*host_nts*sizeof(float)));
		// myz
		cuda_call(cudaMemset(dev_modelRegDts_sigmayz[iGpu], 0, nSourcesRegYZGrid*host_nts*sizeof(float)));
}
...
void setupAdjGpu_3D(float *modelRegDts_vx, float *modelRegDts_vy, float *modelRegDts_vz, float *modelRegDts_sigmaxx, float *modelRegDts_sigmayy, float *modelRegDts_sigmazz, float *modelRegDts_sigmaxz, float *modelRegDts_sigmaxy, float *modelRegDts_sigmayz, float *dataRegDts_vx, float *dataRegDts_vy, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmayy, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, float *dataRegDts_sigmaxy, float *dataRegDts_sigmayz, long long *sourcesPositionRegCenterGrid, long long nSourcesRegCenterGrid, long long *sourcesPositionRegXGrid, long long nSourcesRegXGrid, long long *sourcesPositionRegYGrid, long long nSourcesRegYGrid, long long *sourcesPositionRegZGrid, long long nSourcesRegZGrid, long long *sourcesPositionRegXZGrid, long long nSourcesRegXZGrid, long long *sourcesPositionRegXYGrid, long long nSourcesRegXYGrid, long long *sourcesPositionRegYZGrid, long long nSourcesRegYZGrid, long long *receiversPositionRegCenterGrid, long long nReceiversRegCenterGrid, long long *receiversPositionRegXGrid, long long nReceiversRegXGrid, long long *receiversPositionRegYGrid, long long nReceiversRegYGrid, long long *receiversPositionRegZGrid, long long nReceiversRegZGrid, long long *receiversPositionRegXZGrid, long long nReceiversRegXZGrid, long long *receiversPositionRegXYGrid, long long nReceiversRegXYGrid, long long *receiversPositionRegYZGrid, long long nReceiversRegYZGrid, int nx, int ny, int nz, int iGpu, int iGpuId){
		// Set device number on GPU cluster
		cudaSetDevice(iGpuId);
...
void launchAdjStepKernelsdomDec_3D(dim3 dimGrid, dim3 dimBlock, int nx, int ny, int nz, long long shift, int iGpu, cudaStream_t stream){
		kernel_stream_exec(stepAdjGpu_3D<<<dimGrid, dimBlock, 0, stream>>>(dev_p0_vx[iGpu]+shift, dev_p0_vy[iGpu]+shift, dev_p0_vz[iGpu]+shift, dev_p0_sigmaxx[iGpu]+shift, dev_p0_sigmayy[iGpu]+shift, dev_p0_sigmazz[iGpu]+shift, dev_p0_sigmaxz[iGpu]+shift, dev_p0_sigmaxy[iGpu]+shift, dev_p0_sigmayz[iGpu]+shift, dev_p1_vx[iGpu]+shift, dev_p1_vy[iGpu]+shift, dev_p1_vz[iGpu]+shift, dev_p1_sigmaxx[iGpu]+shift, dev_p1_sigmayy[iGpu]+shift, dev_p1_sigmazz[iGpu]+shift, dev_p1_sigmaxz[iGpu]+shift, dev_p1_sigmaxy[iGpu]+shift, dev_p1_sigmayz[iGpu]+shift, dev_p0_vx[iGpu]+shift, dev_p0_vy[iGpu]+shift, dev_p0_vz[iGpu]+shift, dev_p0_sigmaxx[iGpu]+shift, dev_p0_sigmayy[iGpu]+shift, dev_p0_sigmazz[iGpu]+shift, dev_p0_sigmaxz[iGpu]+shift, dev_p0_sigmaxy[iGpu]+shift, dev_p0_sigmayz[iGpu]+shift, dev_rhoxDtw[iGpu]+shift, dev_rhoyDtw[iGpu]+shift, dev_rhozDtw[iGpu]+shift, dev_lamb2MuDtw[iGpu]+shift, dev_lambDtw[iGpu]+shift, dev_muxzDtw[iGpu]+shift, dev_muxyDtw[iGpu]+shift, dev_muyzDtw[iGpu]+shift, nx, ny, nz));
}
...
void launchAdjExtractSourcedomDec_3D(int nblockSouCenterGrid, int nblockSouXGrid, int nblockSouYGrid, int nblockSouZGrid, int nblockSouXZGrid, int nblockSouXYGrid, int nblockSouYZGrid, long long nSourcesRegCenterGrid, long long nSourcesRegXGrid, long long nSourcesRegYGrid, long long nSourcesRegZGrid, long long nSourcesRegXZGrid, long long nSourcesRegXYGrid, long long nSourcesRegYZGrid, int its, int it2, int iGpu, long long shift, long long min_idx, long long max_idx, cudaStream_t stream){
	// Mxx, Myy, Mzz
	kernel_stream_exec(ker_record_source_centerGriddomDec_3D<<<nblockSouCenterGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_sigmaxx[iGpu], dev_p0_sigmayy[iGpu], dev_p0_sigmazz[iGpu], dev_modelRegDts_sigmaxx[iGpu], dev_modelRegDts_sigmayy[iGpu], dev_modelRegDts_sigmazz[iGpu], its, it2, dev_sourcesPositionRegCenterGrid[iGpu], nSourcesRegCenterGrid, shift, min_idx, max_idx));
	// fx
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouXGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_vx[iGpu], dev_modelRegDts_vx[iGpu], its, it2, dev_sourcesPositionRegXGrid[iGpu], nSourcesRegXGrid, shift, min_idx, max_idx));
	// fy
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouYGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_vy[iGpu], dev_modelRegDts_vy[iGpu], its, it2, dev_sourcesPositionRegYGrid[iGpu], nSourcesRegYGrid, shift, min_idx, max_idx));
	// fz
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouZGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_vz[iGpu], dev_modelRegDts_vz[iGpu], its, it2, dev_sourcesPositionRegZGrid[iGpu], nSourcesRegZGrid, shift, min_idx, max_idx));
	// Mxz
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouXZGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_sigmaxz[iGpu], dev_modelRegDts_sigmaxz[iGpu], its, it2, dev_sourcesPositionRegXZGrid[iGpu], nSourcesRegXZGrid, shift, min_idx, max_idx));
	// Mxy
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouXYGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_sigmaxy[iGpu], dev_modelRegDts_sigmaxy[iGpu], its, it2, dev_sourcesPositionRegXYGrid[iGpu], nSourcesRegXYGrid, shift, min_idx, max_idx));
	// Myz
	kernel_stream_exec(ker_record_source_stagGriddomDec_3D<<<nblockSouYZGrid, BLOCK_SIZE_DATA, 0, stream>>>(dev_p0_sigmayz[iGpu], dev_modelRegDts_sigmayz[iGpu], its, it2, dev_sourcesPositionRegYZGrid[iGpu], nSourcesRegYZGrid, shift, min_idx, max_idx));
...
void propElasticAdjGpu_3D(float *modelRegDts_vx, float *modelRegDts_vy, float *modelRegDts_vz, float *modelRegDts_sigmaxx, float *modelRegDts_sigmayy, float *modelRegDts_sigmazz, float *modelRegDts_sigmaxz, float *modelRegDts_sigmaxy, float *modelRegDts_sigmayz, float *dataRegDts_vx, float *dataRegDts_vy, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmayy, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, float *dataRegDts_sigmaxy, float *dataRegDts_sigmayz, long long *sourcesPositionRegCenterGrid, long long nSourcesRegCenterGrid, long long *sourcesPositionRegXGrid, long long nSourcesRegXGrid, long long *sourcesPositionRegYGrid, long long nSourcesRegYGrid, long long *sourcesPositionRegZGrid, long long nSourcesRegZGrid, long long *sourcesPositionRegXZGrid, long long nSourcesRegXZGrid, long long *sourcesPositionRegXYGrid, long long nSourcesRegXYGrid, long long *sourcesPositionRegYZGrid, long long nSourcesRegYZGrid, long long *receiversPositionRegCenterGrid, long long nReceiversRegCenterGrid, long long *receiversPositionRegXGrid, long long nReceiversRegXGrid, long long *receiversPositionRegYGrid, long long nReceiversRegYGrid, long long *receiversPositionRegZGrid, long long nReceiversRegZGrid, long long *receiversPositionRegXZGrid, long long nReceiversRegXZGrid, long long *receiversPositionRegXYGrid, long long nReceiversRegXYGrid, long long *receiversPositionRegYZGrid, long long nReceiversRegYZGrid, int nx, int ny, int nz, int iGpu, int iGpuId) {
	...
	cuda_call(cudaMemcpy(modelRegDts_vx, dev_modelRegDts_vx[iGpu], nSourcesRegXGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_vy, dev_modelRegDts_vy[iGpu], nSourcesRegYGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_vz, dev_modelRegDts_vz[iGpu], nSourcesRegZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmaxx, dev_modelRegDts_sigmaxx[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmayy, dev_modelRegDts_sigmayy[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmazz, dev_modelRegDts_sigmazz[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmaxz, dev_modelRegDts_sigmaxz[iGpu], nSourcesRegXZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmaxy, dev_modelRegDts_sigmaxy[iGpu], nSourcesRegXYGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
	cuda_call(cudaMemcpy(modelRegDts_sigmayz, dev_modelRegDts_sigmayz[iGpu], nSourcesRegYZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
...
void propElasticAdjGpudomDec_3D(float *modelRegDts_vx, float *modelRegDts_vy, float *modelRegDts_vz, float *modelRegDts_sigmaxx, float *modelRegDts_sigmayy, float *modelRegDts_sigmazz, float *modelRegDts_sigmaxz, float *modelRegDts_sigmaxy, float *modelRegDts_sigmayz, float *dataRegDts_vx, float *dataRegDts_vy, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmayy, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, float *dataRegDts_sigmaxy, float *dataRegDts_sigmayz, long long *sourcesPositionRegCenterGrid, long long nSourcesRegCenterGrid, long long *sourcesPositionRegXGrid, long long nSourcesRegXGrid, long long *sourcesPositionRegYGrid, long long nSourcesRegYGrid, long long *sourcesPositionRegZGrid, long long nSourcesRegZGrid, long long *sourcesPositionRegXZGrid, long long nSourcesRegXZGrid, long long *sourcesPositionRegXYGrid, long long nSourcesRegXYGrid, long long *sourcesPositionRegYZGrid, long long nSourcesRegYZGrid, long long *receiversPositionRegCenterGrid, long long nReceiversRegCenterGrid, long long *receiversPositionRegXGrid, long long nReceiversRegXGrid, long long *receiversPositionRegYGrid, long long nReceiversRegYGrid, long long *receiversPositionRegZGrid, long long nReceiversRegZGrid, long long *receiversPositionRegXZGrid, long long nReceiversRegXZGrid, long long *receiversPositionRegXYGrid, long long nReceiversRegXYGrid, long long *receiversPositionRegYZGrid, long long nReceiversRegYZGrid, int nx, int ny, int nz, std::vector<int> ny_domDec, std::vector<int> gpuList){
  ...
  cudaStream_t haloStream[nGpu], bodyStream[nGpu];
	...
	float* modelRegDts_vxTmp = new float[nSourcesRegXGrid*host_nts];
	float* modelRegDts_vyTmp = new float[nSourcesRegYGrid*host_nts];
	float* modelRegDts_vzTmp = new float[nSourcesRegZGrid*host_nts];
	float* modelRegDts_sigmaxxTmp = new float[nSourcesRegCenterGrid*host_nts];
	float* modelRegDts_sigmayyTmp = new float[nSourcesRegCenterGrid*host_nts];
	float* modelRegDts_sigmazzTmp = new float[nSourcesRegCenterGrid*host_nts];
	float* modelRegDts_sigmaxzTmp = new float[nSourcesRegXZGrid*host_nts];
	float* modelRegDts_sigmaxyTmp = new float[nSourcesRegXYGrid*host_nts];
	float* modelRegDts_sigmayzTmp = new float[nSourcesRegYZGrid*host_nts];
	...
	for (int its = host_nts-2; its > -1; its--){
			for (int it2 = host_sub-1; it2 > -1; it2--){
				...
				/*************************************/
				// Extract and interpolate model (excluding the halos in this case)
				for(int iGpu=0; iGpu<nGpu; iGpu++){
					cudaSetDevice(gpuList[iGpu]);
					launchAdjExtractSourcedomDec_3D(nblockSouCenterGrid, nblockSouXGrid, nblockSouYGrid, nblockSouZGrid,nblockSouXZGrid, nblockSouXYGrid, nblockSouYZGrid, nSourcesRegCenterGrid, nSourcesRegXGrid, nSourcesRegYGrid, nSourcesRegZGrid, nSourcesRegXZGrid, nSourcesRegXYGrid, nSourcesRegYZGrid, its, it2, iGpu, minIdxInj[iGpu], minIdxExt[iGpu], maxIdxExt[iGpu], bodyStream[iGpu]);
				}
				/*************************************/
	...
	for(int iGpu=0; iGpu<nGpu; iGpu++){
		...
		cudaSetDevice(gpuList[iGpu]);
		cuda_call(cudaMemcpy(modelRegDts_vxTmp, dev_modelRegDts_vx[iGpu], nSourcesRegXGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_vyTmp, dev_modelRegDts_vy[iGpu], nSourcesRegYGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_vzTmp, dev_modelRegDts_vz[iGpu], nSourcesRegZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmaxxTmp, dev_modelRegDts_sigmaxx[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmayyTmp, dev_modelRegDts_sigmayy[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmazzTmp, dev_modelRegDts_sigmazz[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmaxzTmp, dev_modelRegDts_sigmaxz[iGpu], nSourcesRegXZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmaxyTmp, dev_modelRegDts_sigmaxy[iGpu], nSourcesRegXYGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDts_sigmayzTmp, dev_modelRegDts_sigmayz[iGpu], nSourcesRegYZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));

		//Adding Vx to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegXGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_vx[idx] += modelRegDts_vxTmp[idx];
			}
		}
		//Adding Vy to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegYGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_vy[idx] += modelRegDts_vyTmp[idx];
			}
		}
		//Adding Vz to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegZGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_vz[idx] += modelRegDts_vzTmp[idx];
			}
		}
		//Adding Sigmaxx, Sigmayy, Sigmazz to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegCenterGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_sigmaxx[idx] += modelRegDts_sigmaxxTmp[idx];
				modelRegDts_sigmayy[idx] += modelRegDts_sigmayyTmp[idx];
				modelRegDts_sigmazz[idx] += modelRegDts_sigmazzTmp[idx];
			}
		}
		//Adding Sigmaxz to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegXZGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_sigmaxz[idx] += modelRegDts_sigmaxzTmp[idx];
			}
		}
		//Adding Sigmaxy to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegXYGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_sigmaxy[idx] += modelRegDts_sigmaxyTmp[idx];
			}
		}
		//Adding Sigmayz to output model
		#pragma omp parallel for collapse(2)
		for (long long idev = 0; idev < nSourcesRegYZGrid; idev++){
			for (int its = 0; its < host_nts; its++){
				long long idx = idev*host_nts + its;
				modelRegDts_sigmayz[idx] += modelRegDts_sigmayzTmp[idx];
			}
		}
	...
	delete[] modelRegDts_sigmaxxTmp;
	delete[] modelRegDts_sigmayyTmp;
	delete[] modelRegDts_sigmazzTmp;
	delete[] modelRegDts_sigmaxzTmp;
	delete[] modelRegDts_sigmaxyTmp;
	delete[] modelRegDts_sigmayzTmp;
