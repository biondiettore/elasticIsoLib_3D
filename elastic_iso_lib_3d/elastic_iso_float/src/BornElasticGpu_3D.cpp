...
bool BornElasticGpu_3D::checkParfileConsistency_3D(const std::shared_ptr<SEP::float4DReg> model, const std::shared_ptr<SEP::float3DReg> data) const{
...
void BornElasticGpu_3D::forward(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float3DReg> data) const {
  ...
  std::shared_ptr<float2DReg> dataIrreg_sigmaxz(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregXZGrid));
	...
	float *drhox_in = model->getVals();
	float *drhoy_in = model->getVals()+nModel;
	float *drhoz_in = model->getVals()+2*nModel;
	float *dlame_in = model->getVals()+3*nModel;
	float *dmu_in   = model->getVals()+4*nModel;
	float *dmuxz_in = model->getVals()+5*nModel;
	float *dmuxy_in = model->getVals()+6*nModel;
	float *dmuyz_in = model->getVals()+7*nModel;
...
void BornElasticGpu_3D::adjoint(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float3DReg> data) const {
