...
//STAGGERED GRIDs
__global__ void ker_inject_source_stagGrid_3D(float *dev_signalIn, float *dev_timeSlice, int its, int it2, long long *dev_sourcesPositionRegGrid, long long nSourcesRegGrid){
...
//STAGGERED GRIDs
__global__ void ker_inject_data_stagGrid_3D(float *dev_signalIn, float *dev_timeSlice, int its, int it2, long long *dev_receiversPositionRegGrid, long long nReceiversRegGrid){
...
__global__ void ker_record_source_centerGrid_3D(float *dev_newTimeSlice_sigmaxx, float *dev_newTimeSlice_sigmayy, float *dev_newTimeSlice_sigmazz, float *dev_signalOut_sigmaxx, float *dev_signalOut_sigmayy, float *dev_signalOut_sigmazz, int its, int it2, long long *dev_sourcesPositionRegCenterGrid, long long nSourcesRegCenterGrid) {
    long long iThread = blockIdx.x * blockDim.x + threadIdx.x;
		if (iThread < nSourcesRegCenterGrid){
...
//STAGGERED GRIDs
__global__ void ker_record_source_stagGrid_3D(float *dev_newTimeSlice, float *dev_signalOut, int its, int it2, long long *dev_sourcesPositionRegGrid, long long nSourcesRegGrid) {
    long long iThread = blockIdx.x * blockDim.x + threadIdx.x;
		if (iThread < nSourcesRegGrid){
...
__global__ void ker_record_interp_data_centerGrid_3D(float *dev_newTimeSlice_sigmaxx, float *dev_newTimeSlice_sigmayy, float *dev_newTimeSlice_sigmazz, float *dev_signalOut_sigmaxx, float *dev_signalOut_sigmayy, float *dev_signalOut_sigmazz, int its, int it2, long long *dev_receiversPositionRegCenterGrid, long long nReceiversRegCenterGrid) {
...
//STAGGERED GRIDs
__global__ void ker_record_interp_data_stagGrid_3D(float *dev_newTimeSlice, float *dev_signalOut, int its, int it2, long long *dev_receiversPositionRegGrid, long long nReceiversRegGrid) {
...
__global__ void stepFwdGpu_3D(float *dev_o_vx, float *dev_o_vy, float *dev_o_vz, float *dev_o_sigmaxx, float *dev_o_sigmayy, float *dev_o_sigmazz, float *dev_o_sigmaxz, float *dev_o_sigmaxy, float *dev_o_sigmayz, float *dev_c_vx, float *dev_c_vy, float *dev_c_vz, float *dev_c_sigmaxx, float *dev_c_sigmayy, float *dev_c_sigmazz, float *dev_c_sigmaxz, float *dev_c_sigmaxy, float *dev_c_sigmayz, float *dev_n_vx, float *dev_n_vy, float *dev_n_vz, float *dev_n_sigmaxx, float *dev_n_sigmayy, float *dev_n_sigmazz, float *dev_n_sigmaxz, float *dev_n_sigmaxy, float *dev_n_sigmayz, float* dev_rhoxDtw, float* dev_rhoyDtw, float* dev_rhozDtw, float* dev_lamb2MuDtw, float* dev_lambDtw, float* dev_muxzDtw, float* dev_muxyDtw, float* dev_muyzDtw, int nx, int ny, int nz){

	// Allocate shared memory for a specific block
	__shared__ float shared_c_vx[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Vx wavefield y-slice block
	__shared__ float shared_c_vy[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Vy wavefield y-slice block
	__shared__ float shared_c_vz[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Vz wavefield y-slice block
	__shared__ float shared_c_sigmaxx[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmaxx wavefield y-slice block
	__shared__ float shared_c_sigmayy[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmayy wavefield y-slice block
	__shared__ float shared_c_sigmazz[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmazz wavefield y-slice block
	__shared__ float shared_c_sigmaxz[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmaxz wavefield y-slice block
	__shared__ float shared_c_sigmaxy[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmaxy wavefield y-slice block
	__shared__ float shared_c_sigmayz[BLOCK_SIZE_X+2*FAT][BLOCK_SIZE_Z+2*FAT];  // Current Sigmayz wavefield y-slice block
	...
	float dev_c_vx_y[2*FAT-1];
	float dev_c_vy_y[2*FAT-1];
	float dev_c_vz_y[2*FAT-1];
	float dev_c_sigmayy_y[2*FAT-1];
	float dev_c_sigmaxy_y[2*FAT-1];
	float dev_c_sigmayz_y[2*FAT-1];
	...
	for (long long iy=FAT; iy<ny-FAT; iy++){
		...
		// dvy/dx (+)
		float dvy_dy = dev_yCoeff[0]*(dev_c_vy_y[3]-shared_c_vy[ixLocal][izLocal]) +
...
/******************************************************************************/
/* Forward stepper (no damping) */
__global__ void stepAdjGpu_3D(float *dev_o_vx, float *dev_o_vy, float *dev_o_vz, float *dev_o_sigmaxx, float *dev_o_sigmayy, float *dev_o_sigmazz, float *dev_o_sigmaxz, float *dev_o_sigmaxy, float *dev_o_sigmayz, float *dev_c_vx, float *dev_c_vy, float *dev_c_vz, float *dev_c_sigmaxx, float *dev_c_sigmayy, float *dev_c_sigmazz, float *dev_c_sigmaxz, float *dev_c_sigmaxy, float *dev_c_sigmayz, float *dev_n_vx, float *dev_n_vy, float *dev_n_vz, float *dev_n_sigmaxx, float *dev_n_sigmayy, float *dev_n_sigmazz, float *dev_n_sigmaxz, float *dev_n_sigmaxy, float *dev_n_sigmayz, float* dev_rhoxDtw, float* dev_rhoyDtw, float* dev_rhozDtw, float* dev_lamb2MuDtw, float* dev_lambDtw, float* dev_muxzDtw, float* dev_muxyDtw, float* dev_muyzDtw, int nx, int ny, int nz){
...
__global__ void stepAdjGpuBorn_3D(float *dev_o_vx, float *dev_o_vy, float *dev_o_vz, float *dev_o_sigmaxx, float *dev_o_sigmayy, float *dev_o_sigmazz, float *dev_o_sigmaxz, float *dev_o_sigmaxy, float *dev_o_sigmayz, float *dev_c_vx, float *dev_c_vy, float *dev_c_vz, float *dev_c_sigmaxx, float *dev_c_sigmayy, float *dev_c_sigmazz, float *dev_c_sigmaxz, float *dev_c_sigmaxy, float *dev_c_sigmayz, float *dev_n_vx, float *dev_n_vy, float *dev_n_vz, float *dev_n_sigmaxx, float *dev_n_sigmayy, float *dev_n_sigmazz, float *dev_n_sigmaxz, float *dev_n_sigmaxy, float *dev_n_sigmayz, float* dev_rhoxDtw, float* dev_rhoyDtw, float* dev_rhozDtw, float* dev_lamb2MuDtw, float* dev_lambDtw, float* dev_muxzDtw, float* dev_muxyDtw, float* dev_muyzDtw, int nx, int ny, int nz){
	...
	long long iGlobalVx = iGlobal - FAT * yStride;
	// Loading stride for Vx along the y-direction (backward derivative)
	dev_c_vx_y[1] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // iy = 0
	dev_c_vx_y[2] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // iy = 1
	dev_c_vx_y[3] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // iy = 2
	shared_c_vx[ixLocal][izLocal] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 3
	dev_c_vx_y[4] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // iy = 4
	dev_c_vx_y[5] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];	iGlobalVx+=yStride; // iy = 5
	dev_c_vx_y[6] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx]; // iy = 6

	// Loading for Vy along the y-direction (forward derivative)
	long long iGlobalVy = iGlobal - (FAT-1) * yStride;
	dev_c_vy_y[1] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride; // iy = 0
	dev_c_vy_y[2] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride; // iy = 1
	shared_c_vy[ixLocal][izLocal] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 2
	dev_c_vy_y[3] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride; // iy = 3
	dev_c_vy_y[4] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride; // iy = 4
	dev_c_vy_y[5] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; iGlobalVy+=yStride;// iy = 5
	dev_c_vy_y[6] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy]; // At that point, iyTemp = 2*FAT-1 // iy = 6

	// Loading for Vz along the y-direction (backward derivative)
	long long iGlobalVz = iGlobal - FAT * yStride;
	dev_c_vz_y[1] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // iy = 0
	dev_c_vz_y[2] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // iy = 1
	dev_c_vz_y[3] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // iy = 2
	shared_c_vz[ixLocal][izLocal] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 3
	dev_c_vz_y[4] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // iy = 4
	dev_c_vz_y[5] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; iGlobalVz+=yStride; // iy = 5
	dev_c_vz_y[6] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz]; // iy = 6

	// Loading for Sigmaxy along the y-direction (forward derivative)
	long long iGlobalSxy = iGlobal - (FAT-1) * yStride;
	dev_c_sigmaxy_y[1] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // iy = 0
	dev_c_sigmaxy_y[2] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // iy = 1
	shared_c_sigmaxy[ixLocal][izLocal] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 2
	dev_c_sigmaxy_y[3] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // iy = 3
	dev_c_sigmaxy_y[4] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // iy = 4
	dev_c_sigmaxy_y[5] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; iGlobalSxy+=yStride; // iy = 5
	dev_c_sigmaxy_y[6] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy]; // At that point, iyTemp = 2*FAT-1 // iy = 6

	// Loading for Sigmayy along the y-direction (backward derivative)
	long long iGlobalSyy = iGlobal - FAT * yStride;
	dev_c_sigmayy_y[1] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // iy = 0
	dev_c_sigmayy_y[2] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // iy = 1
	dev_c_sigmayy_y[3] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // iy = 2
	shared_c_sigmayy[ixLocal][izLocal] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 3
	dev_c_sigmayy_y[4] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // iy = 4
	dev_c_sigmayy_y[5] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; iGlobalSyy+=yStride; // iy = 5
	dev_c_sigmayy_y[6] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy]; // iy = 6

	// Loading for Sigmayz along the y-direction (forward derivative)
	long long iGlobalSyz = iGlobal - (FAT-1) * yStride;
	dev_c_sigmayz_y[1] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // iy = 0
	dev_c_sigmayz_y[2] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // iy = 1
	shared_c_sigmayz[ixLocal][izLocal] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // Only the central point on the y-axis is stored in the shared memory // iy = 2
	dev_c_sigmayz_y[3] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // iy = 3
	dev_c_sigmayz_y[4] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // iy = 4
	dev_c_sigmayz_y[5] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; iGlobalSyz+=yStride; // iy = 5
	dev_c_sigmayz_y[6] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; // At that point, iyTemp = 2*FAT-1 // iy = 6
	...
	for (long long iy=FAT; iy<ny-FAT; iy++){
		...
		dev_c_vx_y[5] = dev_c_vx_y[6];
		iGlobalVx+=yStride;
		dev_c_vx_y[6] = dev_c_vx[iGlobalVx]*dev_rhoxDtw[iGlobalVx];
		...
		dev_c_vy_y[5] = dev_c_vy_y[6];
		iGlobalVy+=yStride;
		dev_c_vy_y[6] = dev_c_vy[iGlobalVy]*dev_rhoyDtw[iGlobalVy];
		...
		dev_c_vz_y[5] = dev_c_vz_y[6];
		iGlobalVz+=yStride;
		dev_c_vz_y[6] = dev_c_vz[iGlobalVz]*dev_rhozDtw[iGlobalVz];
		...
		dev_c_sigmaxy_y[5] = dev_c_sigmaxy_y[6];
		iGlobalSxy+=yStride;
		dev_c_sigmaxy_y[6] = dev_c_sigmaxy[iGlobalSxy]*dev_muxyDtw[iGlobalSxy];
		...
		dev_c_sigmayy_y[5] = dev_c_sigmayy_y[6];
		iGlobalSyy+=yStride;
		dev_c_sigmayy_y[6] = dev_c_sigmayy[iGlobalSyy]*dev_lamb2MuDtw[iGlobalSyy]+(dev_c_sigmaxx[iGlobalSyy]+dev_c_sigmazz[iGlobalSyy])*dev_lambDtw[iGlobalSyy];
		...
		dev_c_sigmayz_y[5] = dev_c_sigmayz_y[6];
		iGlobalSyz+=yStride;
		dev_c_sigmayz_y[6] = dev_c_sigmayz[iGlobalSyz]*dev_muyzDtw[iGlobalSyz]; // The last point of the stencil now points to the next y-slice

		// Loading fields that does not require y-derivate
		shared_c_sigmaxx[ixLocal][izLocal] = dev_c_sigmaxx[iGlobal]*dev_lamb2MuDtw[iGlobal]+(dev_c_sigmayy[iGlobal]+dev_c_sigmazz[iGlobal])*dev_lambDtw[iGlobal]; // sigmaxx
		shared_c_sigmazz[ixLocal][izLocal] = dev_c_sigmazz[iGlobal]*dev_lamb2MuDtw[iGlobal]+(dev_c_sigmaxx[iGlobal]+dev_c_sigmayy[iGlobal])*dev_lambDtw[iGlobal]; // sigmazz
		shared_c_sigmaxz[ixLocal][izLocal] = dev_c_sigmaxz[iGlobal]*dev_muxzDtw[iGlobal]; // sigmaxz

		// Load the halos in the x-direction
		// Threads with x-index ranging from 0,...,FAT will load the first and last FAT elements of the block on the x-axis to shared memory
		if (threadIdx.y < FAT) {
			shared_c_vx[threadIdx.y][izLocal] = dev_c_vx[iGlobal-nz*FAT]*dev_rhoxDtw[iGlobal-nz*FAT]; // Left side
			shared_c_vy[threadIdx.y][izLocal] = dev_c_vy[iGlobal-nz*FAT]*dev_rhoyDtw[iGlobal-nz*FAT]; // Left side
			shared_c_vz[threadIdx.y][izLocal] = dev_c_vz[iGlobal-nz*FAT]*dev_rhozDtw[iGlobal-nz*FAT]; // Left side
			shared_c_sigmaxx[threadIdx.y][izLocal] = dev_c_sigmaxx[iGlobal-nz*FAT]*dev_lamb2MuDtw[iGlobal-nz*FAT]+(dev_c_sigmayy[iGlobal-nz*FAT]+dev_c_sigmazz[iGlobal-nz*FAT])*dev_lambDtw[iGlobal-nz*FAT]; // Left side
			// shared_c_sigmayy[threadIdx.y][izLocal] = dev_c_sigmayy[iGlobal-nz*FAT]; // Left side
			shared_c_sigmazz[threadIdx.y][izLocal] = dev_c_sigmazz[iGlobal-nz*FAT]*dev_lamb2MuDtw[iGlobal-nz*FAT]+(dev_c_sigmaxx[iGlobal-nz*FAT]+dev_c_sigmayy[iGlobal-nz*FAT])*dev_lambDtw[iGlobal-nz*FAT]; // Left side
			shared_c_sigmaxz[threadIdx.y][izLocal] = dev_c_sigmaxz[iGlobal-nz*FAT]*dev_muxzDtw[iGlobal-nz*FAT]; // Left side
			shared_c_sigmaxy[threadIdx.y][izLocal] = dev_c_sigmaxy[iGlobal-nz*FAT]*dev_muxyDtw[iGlobal-nz*FAT]; // Left side
			shared_c_sigmayz[threadIdx.y][izLocal] = dev_c_sigmayz[iGlobal-nz*FAT]*dev_muyzDtw[iGlobal-nz*FAT]; // Left side

			shared_c_vx[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_vx[iGlobal+nz*BLOCK_SIZE_X]*dev_rhoxDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_vy[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_vy[iGlobal+nz*BLOCK_SIZE_X]*dev_rhoyDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_vz[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_vz[iGlobal+nz*BLOCK_SIZE_X]*dev_rhozDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_sigmaxx[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmaxx[iGlobal+nz*BLOCK_SIZE_X]*dev_lamb2MuDtw[iGlobal+nz*BLOCK_SIZE_X]+(dev_c_sigmayy[iGlobal+nz*BLOCK_SIZE_X]+dev_c_sigmazz[iGlobal+nz*BLOCK_SIZE_X])*dev_lambDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			// shared_c_sigmayy[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmayy[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_sigmazz[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmazz[iGlobal+nz*BLOCK_SIZE_X]*dev_lamb2MuDtw[iGlobal+nz*BLOCK_SIZE_X]+(dev_c_sigmaxx[iGlobal+nz*BLOCK_SIZE_X]+dev_c_sigmayy[iGlobal+nz*BLOCK_SIZE_X])*dev_lambDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_sigmaxz[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmaxz[iGlobal+nz*BLOCK_SIZE_X]*dev_muxzDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_sigmaxy[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmaxy[iGlobal+nz*BLOCK_SIZE_X]*dev_muxyDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
			shared_c_sigmayz[ixLocal+BLOCK_SIZE_X][izLocal] = dev_c_sigmayz[iGlobal+nz*BLOCK_SIZE_X]*dev_muyzDtw[iGlobal+nz*BLOCK_SIZE_X]; // Right side
		}

		// Load the halos in the z-direction
		if (threadIdx.x < FAT) {
			shared_c_vx[ixLocal][threadIdx.x] = dev_c_vx[iGlobal-FAT]*dev_rhoxDtw[iGlobal-FAT]; // Up
			shared_c_vy[ixLocal][threadIdx.x] = dev_c_vy[iGlobal-FAT]*dev_rhoyDtw[iGlobal-FAT]; // Up
			shared_c_vz[ixLocal][threadIdx.x] = dev_c_vz[iGlobal-FAT]*dev_rhozDtw[iGlobal-FAT]; // Up
			shared_c_sigmaxx[ixLocal][threadIdx.x] = dev_c_sigmaxx[iGlobal-FAT]*dev_lamb2MuDtw[iGlobal-FAT]+(dev_c_sigmayy[iGlobal-FAT]+dev_c_sigmazz[iGlobal-FAT])*dev_lambDtw[iGlobal-FAT]; // Up
			// shared_c_sigmayy[ixLocal][threadIdx.x] = dev_c_sigmayy[iGlobal-FAT]; // Up
			shared_c_sigmazz[ixLocal][threadIdx.x] = dev_c_sigmazz[iGlobal-FAT]*dev_lamb2MuDtw[iGlobal-FAT]+(dev_c_sigmaxx[iGlobal-FAT]+dev_c_sigmayy[iGlobal-FAT])*dev_lambDtw[iGlobal-FAT]; // Up
			shared_c_sigmaxz[ixLocal][threadIdx.x] = dev_c_sigmaxz[iGlobal-FAT]*dev_muxzDtw[iGlobal-FAT]; // Up
			shared_c_sigmaxy[ixLocal][threadIdx.x] = dev_c_sigmaxy[iGlobal-FAT]*dev_muxyDtw[iGlobal-FAT]; // Up
			shared_c_sigmayz[ixLocal][threadIdx.x] = dev_c_sigmayz[iGlobal-FAT]*dev_muyzDtw[iGlobal-FAT]; // Up

			shared_c_vx[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_vx[iGlobal+BLOCK_SIZE_Z]*dev_rhoxDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_vy[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_vy[iGlobal+BLOCK_SIZE_Z]*dev_rhoyDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_vz[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_vz[iGlobal+BLOCK_SIZE_Z]*dev_rhozDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_sigmaxx[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmaxx[iGlobal+BLOCK_SIZE_Z]*dev_lamb2MuDtw[iGlobal+BLOCK_SIZE_Z]+(dev_c_sigmayy[iGlobal+BLOCK_SIZE_Z]+dev_c_sigmazz[iGlobal+BLOCK_SIZE_Z])*dev_lambDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			// shared_c_sigmayy[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmayy[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_sigmazz[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmazz[iGlobal+BLOCK_SIZE_Z]*dev_lamb2MuDtw[iGlobal+BLOCK_SIZE_Z]+(dev_c_sigmaxx[iGlobal+BLOCK_SIZE_Z]+dev_c_sigmayy[iGlobal+BLOCK_SIZE_Z])*dev_lambDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_sigmaxz[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmaxz[iGlobal+BLOCK_SIZE_Z]*dev_muxzDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_sigmaxy[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmaxy[iGlobal+BLOCK_SIZE_Z]*dev_muxyDtw[iGlobal+BLOCK_SIZE_Z]; // Down
			shared_c_sigmayz[ixLocal][izLocal+BLOCK_SIZE_Z] = dev_c_sigmayz[iGlobal+BLOCK_SIZE_Z]*dev_muyzDtw[iGlobal+BLOCK_SIZE_Z]; // Down
		}
		...
		// Updating particle velocity fields
		// Vx
		dev_o_vx[iGlobal] = dev_n_vx[iGlobal] - (
													// dsigmaxx/dx (-)
													dev_xCoeff[0]*(shared_c_sigmaxx[ixLocal][izLocal]-shared_c_sigmaxx[ixLocal-1][izLocal])+
										      dev_xCoeff[1]*(shared_c_sigmaxx[ixLocal+1][izLocal]-shared_c_sigmaxx[ixLocal-2][izLocal])+
										      dev_xCoeff[2]*(shared_c_sigmaxx[ixLocal+2][izLocal]-shared_c_sigmaxx[ixLocal-3][izLocal])+
		...
		// Vy
		dev_o_vy[iGlobal] = dev_n_vy[iGlobal] - (
													// dsigmaxy/dx (+)
													dev_xCoeff[0]*(shared_c_sigmaxy[ixLocal+1][izLocal]-shared_c_sigmaxy[ixLocal][izLocal])+
										      dev_xCoeff[1]*(shared_c_sigmaxy[ixLocal+2][izLocal]-shared_c_sigmaxy[ixLocal-1][izLocal])+
										      dev_xCoeff[2]*(shared_c_sigmaxy[ixLocal+3][izLocal]-shared_c_sigmaxy[ixLocal-2][izLocal])+
		...
		// Vz
		dev_o_vz[iGlobal] = dev_n_vz[iGlobal] - (
													// dsigmaxz/dx (+)
													dev_xCoeff[0]*(shared_c_sigmaxz[ixLocal+1][izLocal]-shared_c_sigmaxz[ixLocal][izLocal])+
										      dev_xCoeff[1]*(shared_c_sigmaxz[ixLocal+2][izLocal]-shared_c_sigmaxz[ixLocal-1][izLocal])+
										      dev_xCoeff[2]*(shared_c_sigmaxz[ixLocal+3][izLocal]-shared_c_sigmaxz[ixLocal-2][izLocal])+
...
/************************************** Damping *******************************/
/******************************************************************************/
__global__ void dampCosineEdge_3D(float *dev_p1_vx, float *dev_p2_vx, float *dev_p1_vy, float *dev_p2_vy, float *dev_p1_vz, float *dev_p2_vz, float *dev_p1_sigmaxx, float *dev_p2_sigmaxx, float *dev_p1_sigmayy, float *dev_p2_sigmayy, float *dev_p1_sigmazz, float *dev_p2_sigmazz, float *dev_p1_sigmaxz, float *dev_p2_sigmaxz, float *dev_p1_sigmaxy, float *dev_p2_sigmaxy, float *dev_p1_sigmayz, float *dev_p2_sigmayz, int nx, int ny, int nz) {
...
/**************************** Scattering/Imaging ******************************/
/******************************************************************************/
__global__ void imagingElaFwdGpu_3D(float* dev_o_vx, float* dev_c_vx, float* dev_n_vx, float* dev_o_vy, float* dev_c_vy, float* dev_n_vy, float* dev_o_vz, float* dev_c_vz, float* dev_n_vz, float* dev_vx, float* dev_vy, float* dev_vz, float* dev_sigmaxx, float* dev_sigmayy, float* dev_sigmazz, float* dev_sigmaxz, float* dev_sigmaxy, float* dev_sigmayz, float* dev_drhox, float* dev_drhoy, float* dev_drhoz, float* dev_dlame, float* dev_dmu, float* dev_dmuxz, float* dev_dmuxy, float* dev_dmuyz, float* dev_muxzDtw, float* dev_muxyDtw, float* dev_muyzDtw, int nx, int ny, int nz, int its){
    ...
    if(its == 0){
		...
		float dvxyz_dxyz = dvx_dx + dvy_dy + dvz_dz;
...
__global__ void imagingElaAdjGpu_3D(float* dev_o_vx, float* dev_c_vx, float* dev_n_vx, float* dev_o_vy, float* dev_c_vy, float* dev_n_vy, float* dev_o_vz, float* dev_c_vz, float* dev_n_vz, float* dev_vx, float* dev_vy, float* dev_vz, float* dev_sigmaxx, float* dev_sigmayy, float* dev_sigmazz, float* dev_sigmaxz, float* dev_sigmaxy, float* dev_sigmayz, float* dev_drhox, float* dev_drhoy, float* dev_drhoz, float* dev_dlame, float* dev_dmu, float* dev_dmuxz, float* dev_dmuxy, float* dev_dmuyz, float* dev_lamb2MuDtw, float* dev_lambDtw, float* dev_muxzDtw, float* dev_muxyDtw, float* dev_muyzDtw, int nx, int ny, int nz, int its){
...
/****************************************************************************************/
__global__ void interpWavefieldVxVyVz_3D(float *dev_wavefieldVx_left, float *dev_wavefieldVx_right, float *dev_wavefieldVy_left, float *dev_wavefieldVy_right, float *dev_wavefieldVz_left, float *dev_wavefieldVz_right, float *dev_timeSliceVx, float *dev_timeSliceVy, float *dev_timeSliceVz, int nx, int ny, int nz, int it2) {
...
__global__ void injectSecondarySource_3D(float *dev_ssLeft, float *dev_ssRight, float *dev_p0, int nx, int ny, int nz, int indexFilter){
	long long izGlobal = FAT + blockIdx.x * BLOCK_SIZE + threadIdx.x; // Global z-coordinate
	long long ixGlobal = FAT + blockIdx.y * BLOCK_SIZE + threadIdx.y; // Global x-coordinate
...
__global__ void extractInterpAdjointWavefield_3D(float *dev_timeSliceLeft, float *dev_timeSliceRight, float *dev_timeSliceFine, int nx, int ny, int nz, int it2) {
