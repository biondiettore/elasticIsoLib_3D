...
class spaceInterpGpu_3D : public Operator<SEP::float2DReg, SEP::float2DReg> {
	...
	public:
		...
		spaceInterpGpu_3D(const std::shared_ptr<float1DReg> zCoord, const std::shared_ptr<float1DReg> xCoord, const std::shared_ptr<float1DReg> yCoord, const std::shared_ptr<SEP::hypercube> elasticParamHypercube, int &nt, std::shared_ptr<paramObj> par, int dipole=0, float zDipoleShift=0, float xDipoleShift=0, float yDipoleShift=0, std::string interpMethod="linear", int hFilter1d=1);
