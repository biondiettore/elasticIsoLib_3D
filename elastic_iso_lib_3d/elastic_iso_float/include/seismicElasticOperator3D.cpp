...
void seismicElasticOperator3D <V1, V2>::setSources_3D(std::shared_ptr<spaceInterpGpu_3D> sourcesCenterGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesXGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesYGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesZGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesXZGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesXYGrid, std::shared_ptr<spaceInterpGpu_3D> sourcesYZGrid, std::shared_ptr<V2> sourcesSignals){
  ...
  _nSourcesIrregCenterGrid = _sourcesCenterGrid->getNDeviceIrreg();
	...
	/* Allocation */
  std::shared_ptr<float2DReg> sourceIrreg_vx(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregXGrid));
	std::shared_ptr<float2DReg> sourceIrreg_vy(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregYGrid));
  std::shared_ptr<float2DReg> sourceIrreg_vz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregZGrid));
  std::shared_ptr<float2DReg> sourceIrreg_sigmaxx(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregCenterGrid));
	std::shared_ptr<float2DReg> sourceIrreg_sigmayy(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregCenterGrid));
  std::shared_ptr<float2DReg> sourceIrreg_sigmazz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregCenterGrid));
  std::shared_ptr<float2DReg> sourceIrreg_sigmaxz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregXZGrid));
	std::shared_ptr<float2DReg> sourceIrreg_sigmaxy(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregXYGrid));
	std::shared_ptr<float2DReg> sourceIrreg_sigmayz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregYZGrid));

	_sourceRegDtw_vx = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegXGrid);
	_sourceRegDtw_vy = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegYGrid);
	_sourceRegDtw_vz = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegZGrid);
	_sourceRegDtw_sigmaxx = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegCenterGrid);
	_sourceRegDtw_sigmayy = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegCenterGrid);
	_sourceRegDtw_sigmazz = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegCenterGrid);
	_sourceRegDtw_sigmaxz = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegXZGrid);
	_sourceRegDtw_sigmaxy = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegXYGrid);
	_sourceRegDtw_sigmayz = std::make_shared<float2DReg>(_fdParamElastic->_nts, _nSourcesRegYZGrid);

	/* Copy from 3d source to respective 2d source components */
	// fx
	long long shift;
	long long chunk = _nSourcesIrregXGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_vx->getVals(), _sourcesSignals->getVals(), chunk*sizeof(float));
	// fy
	shift = chunk;
	chunk = _nSourcesIrregYGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_vy->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// fz
	shift += chunk;
	chunk = _nSourcesIrregZGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_vz->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// mxx
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmaxx->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// myy
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmayy->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// mzz
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmazz->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// mxz
	shift += chunk;
	chunk = _nSourcesIrregXZGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmaxz->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// mxz
	shift += chunk;
	chunk = _nSourcesIrregXYGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmaxy->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
	// myz
	shift += chunk;
	chunk = _nSourcesIrregYZGrid*_fdParamElastic->_nts;
  std::memcpy(sourceIrreg_sigmayz->getVals(), _sourcesSignals->getVals()+shift, chunk*sizeof(float));
...
// Wavefield setup
template <class V1, class V2>
std::shared_ptr<float5DReg> seismicElasticOperator3D <V1, V2>:: setWavefield_3D(int wavefieldFlag){

	_saveWavefield = wavefieldFlag;

	std::shared_ptr<float5DReg> wavefield;
	if (wavefieldFlag == 1) {
		wavefield = std::make_shared<float5DReg>(_fdParamElastic->_zAxis, _fdParamElastic->_yAxis, _fdParamElastic->_xAxis, _fdParamElastic->_wavefieldCompAxis, _fdParamElastic->_timeAxisCoarse);
		unsigned long long int wavefieldSize = _fdParamElastic->_zAxis.n * _fdParamElastic->_yAxis.n * _fdParamElastic->_xAxis.n;
		wavefieldSize *= _fdParamElastic->_wavefieldCompAxis.n *_fdParamElastic->_nts*sizeof(float);
		memset(wavefield->getVals(), 0, wavefieldSize);
		return wavefield;
	}
	else {
		wavefield = std::make_shared<float5DReg>(1, 1, 1, 1, 1);
