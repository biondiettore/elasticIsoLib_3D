...
class Operator
{
	...
	public:
		...
		bool dotTest(const bool verbose = false, const float maxError = .00001) const
		{
			...
			float dot1 = r1->dot(r2), dot2 = d1->dot(d2);
			float errorNoAdd = fabs((dot1 - dot2)/dot2); /* fabs is absolute value for float */
			if(verbose)
			{
			...
			forward(true, d1, r2);
			adjoint(true, d2, r1);
			float dot3 = r1->dot(r2), dot4 = d1->dot(d2);
			float errorAdd = fabs((dot3 - dot4)/dot4);
			if(verbose)
			{
