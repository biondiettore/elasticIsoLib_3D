...
*/
class nonlinearPropElasticGpu_3D : public seismicElasticOperator3D<SEP::float3DReg, SEP::float3DReg> {

	protected:

		std::shared_ptr<float5DReg> _wavefield;
    ...
    //! Constructor.
  	...
  	//! QC
		virtual bool checkParfileConsistency_3D(std::shared_ptr<SEP::float3DReg> model, std::shared_ptr<SEP::float3DReg> data) const;
	  ...
	  //! ADJ
		...
		//! Accesor
		std::shared_ptr<float5DReg> getWavefield_3D() { return _wavefield; }
