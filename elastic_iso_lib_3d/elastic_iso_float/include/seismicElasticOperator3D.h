...
class seismicElasticOperator3D : public Operator <V1, V2> {
    ...
    //these variables hold all five components of elastic source signal. Should be a 3d reg
		...
		std::shared_ptr<float2DReg> _sourceRegDtw_vx, _sourceRegDtw_vy, _sourceRegDtw_vz, _sourceRegDtw_sigmaxx, _sourceRegDtw_sigmayy, _sourceRegDtw_sigmazz, _sourceRegDtw_sigmaxz, _sourceRegDtw_sigmaxy, _sourceRegDtw_sigmayz;
	...
	public:
		...
		std::shared_ptr<float5DReg> setWavefield_3D(int wavefieldFlag); // Allocates and returns a wavefield if flag = 1
