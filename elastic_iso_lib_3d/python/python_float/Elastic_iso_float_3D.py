...
import pyElastic_iso_float_nl_3D
import pyElastic_iso_float_born_3D
...
from pyElastic_iso_float_nl_3D import spaceInterpGpu_3D
...
def buildSourceGeometry_3D(parObject, elasticParam):
  ...
  if (nExp == -1):
    raise ValueError(
        "**** ERROR [buildSourceGeometry_3D]: User must provide the total number of shots ****\n"
    )
  ...
  if (spaceInterpMethod == "linear"):
    ...
    raise ValueError(
        "**** ERROR [buildSourceGeometry_3D]: Spatial interpolation method requested by user is not implemented ****\n"
    )

  # Display information for user
  if (info == 1):
    print(
        "**** [buildSourceGeometry_3D]: User has requested to display information ****\n"
    )
    # Interpolation method
    if (spaceInterpMethod == "sinc"):
      print(
          "**** [buildSourceGeometry_3D]: User has requested a sinc spatial interpolation method for the sources' signals injection/extraction ****\n"
      )
    else:
      print(
          "**** [buildSourceGeometry_3D]: User has requested a linear spatial interpolation method for the sources' signals injection/extraction ****\n"
      )
    # Dipole injection/extraction
    if (dipole == 1):
      print(
          "**** [buildSourceGeometry_3D]: User has requested a dipole source injection/extraction ****\n"
      )
      print(
          "**** [buildSourceGeometry_3D]: Dipole shift in z-direction: %f [km or m]  ****\n"
          % zDipoleShift)
      print(
          "**** [buildSourceGeometry_3D]: Dipole shift in x-direction: %f [km or m]  ****\n"
          % xDipoleShift)
      print(
          "**** [buildSourceGeometry_3D]: Dipole shift in y-direction: %f [km or m]  ****\n"
          % yDipoleShift)
  ...
  xzGridHyper = Hypercube.hypercube(
      axes=[zAxisShifted, xAxisShifted, yAxis, paramAxis])
  xyGridHyper = Hypercube.hypercube(
      axes=[zAxis, xAxisShifted, yAxisShifted, paramAxis])
  yzGridHyper = Hypercube.hypercube(
      axes=[zAxisShifted, xAxis, yAxisShifted, paramAxis])
  ...
  for iExp in range(nExp):
    zCoord = SepVector.getSepVector(ns=[nSimSou], storage="dataFloat")
    xCoord = SepVector.getSepVector(ns=[nSimSou], storage="dataFloat")
    yCoord = SepVector.getSepVector(ns=[nSimSou], storage="dataFloat")
    ...
    sourcesVectorCenterGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          centerGridHyper.getCpp(), nts, parObject.param,
                          dipole, zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    # Staggered grids for forces
    sourcesVectorXGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          xGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    sourcesVectorYGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          yGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    sourcesVectorZGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          zGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    # Staggered grids for stresses
    sourcesVectorXZGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          xzGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    sourcesVectorXYGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          xyGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    sourcesVectorYZGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          yzGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
...
def buildReceiversGeometry_3D(parObject, elasticParam):
  ...
  if (nExp == -1):
    raise ValueError(
        "**** ERROR [buildReceiversGeometry_3D]: User must provide the total number of shots ****\n"
    )
  ...
  if (nReceiverPerShot == -1):
    raise ValueError(
        "**** ERROR [buildReceiversGeometry_3D]: User must provide the total number of receivers per shot (this number must be the same for all shots [nReceiverPerShot]) ****\n"
    )
  ...
  if (spaceInterpMethod == "linear"):
    ...
    raise ValueError(
        "**** ERROR [buildReceiversGeometry_3D]: Spatial interpolation method requested by user is not implemented ****\n"
    )
  ...
  if (info == 1):
    ...
    if (spaceInterpMethod == "sinc"):
      print(
          "**** [buildReceiversGeometry_3D]: User has requested a sinc spatial interpolation method for receivers' signals injection/extraction ****\n"
      )
    else:
      print(
          "**** [buildReceiversGeometry_3D]: User has requested a linear spatial interpolation method for receivers' signals injection/extraction ****\n"
      )
    if (dipole == 1):
      print(
          "**** [buildReceiversGeometry_3D]: User has requested a dipole source injection/extraction ****\n"
      )
      print(
          "**** [buildReceiversGeometry_3D]: Dipole shift in z-direction: %f [km or m]  ****\n"
          % zDipoleShift)
      print(
          "**** [buildReceiversGeometry_3D]: Dipole shift in x-direction: %f [km or m]  ****\n"
          % xDipoleShift)
      print(
          "**** [buildReceiversGeometry_3D]: Dipole shift in y-direction: %f [km or m]  ****\n"
          % yDipoleShift)
  ...
  # Third (slowest) axis: spatial coordinates [x,y,z]
  receiverGeomVectorNd = genericIO.defaultIO.getVector(receiverGeomFile,
                                                       ndims=3).getNdArray()
  # Check consistency with total number of shots
  if (receiverGeomVectorNd.shape[2] != 1 and
      nExp != receiverGeomVectorNd.shape[2]):
    raise ValueError(
        "**** ERROR [buildReceiversGeometry_3D]: Number of experiments from parfile (#shot=%s) not consistent with receivers' geometry file (#shots=%s) ****\n"
        % (nExp, receiverGeomVectorNd.shape[2]))
  ...
  if (nReceiverPerShot != receiverGeomVectorNd.shape[1]):
    raise ValueError(
        "**** ERROR [buildReceiversGeometry_3D]: Number of receivers from parfile (#receivers=%s) not consistent with receivers' geometry file (#receivers=%s) ****\n"
        % (nReceiverPerShot, receiverGeomVectorNd.shape[1]))

  nExp = receiverGeomVectorNd.shape[2]
  if (nExp == 1 and info == 1):
    print(
        "**** [buildReceiversGeometry_3D]: User has requested a constant geometry (over shots) for receivers ****\n"
    )
  ...
  for iExp in range(nExp):
    ...
    zCoord = SepVector.getSepVector(ns=[nReceiverPerShot], storage="dataFloat")
    xCoord = SepVector.getSepVector(ns=[nReceiverPerShot], storage="dataFloat")
    yCoord = SepVector.getSepVector(ns=[nReceiverPerShot], storage="dataFloat")
    ...
    recVectorXZGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          ...
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    recVectorXYGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          xyGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
    recVectorYZGrid.append(
        spaceInterpGpu_3D(zCoord.getCpp(), xCoord.getCpp(), yCoord.getCpp(),
                          yzGridHyper.getCpp(), nts, parObject.param, dipole,
                          zDipoleShift, xDipoleShift, yDipoleShift,
                          spaceInterpMethod, hFilter1d))
...
class nonlinearPropElasticShotsGpu_3D(Op.Operator):
  ...
  def __init__(self, domain, range, elasticParam, paramP,
               ...
               sourcesVectorYZGrid, recVectorCenterGrid, recVectorXGrid,
               recVectorYGrid, recVectorZGrid, recVectorXZGrid, recVectorXYGrid,
               recVectorYZGrid):
    ...
    self.pyOp = pyElastic_iso_float_nl_3D.nonlinearPropElasticShotsGpu_3D(
...
def nonlinearFwiOpInitFloat_3D(args):
  ...
  sourceGeomVector = genericIO.defaultIO.getVector(sourceGeomFile, ndims=3)
  sourceSimAxis = sourceGeomVector.getHyper().getAxis(2)
  sourcesHyper = Hypercube.hypercube(
      axes=[timeAxis, sourceSimAxis, wavefieldAxis, dummyAxis])
...
class nonlinearFwiPropElasticShotsGpu_3D(Op.Operator):
  ...
  def __init__(self, domain, range, sources, paramP, sourcesVectorCenterGrid,
               sourcesVectorXGrid, sourcesVectorYGrid, sourcesVectorZGrid,
               sourcesVectorXZGrid, sourcesVectorXYGrid, sourcesVectorYZGrid,
               recVectorCenterGrid, recVectorXGrid, recVectorYGrid,
               recVectorZGrid, recVectorXZGrid, recVectorXYGrid,
               recVectorYZGrid):
    #Domain = source wavelet
    #Range = recorded data space
    self.setDomainRange(domain, range)
    ...
    self.pyOp = pyElastic_iso_float_nl_3D.nonlinearPropElasticShotsGpu_3D(
        domain, paramP, sourcesVectorCenterGrid, sourcesVectorXGrid,
        sourcesVectorYGrid, sourcesVectorZGrid, sourcesVectorXZGrid,
        sourcesVectorXYGrid, sourcesVectorYZGrid, recVectorCenterGrid,
        recVectorXGrid, recVectorYGrid, recVectorZGrid, recVectorXZGrid,
        recVectorXYGrid, recVectorYZGrid)
...
def BornOpInitFloat_3D(args):
  ...
  sourcesSignalsInput = genericIO.defaultIO.getVector(sourcesFile, ndims=4)
  sourcesSignalsFloat = SepVector.getSepVector(sourceHyper, storage="dataFloat")
  sourcesSignalsFloatNp = sourcesSignalsFloat.getNdArray()
  sourcesSignalsInputNp = sourcesSignalsInput.getNdArray()
  sourcesSignalsFloatNp.flat[:] = sourcesSignalsInputNp
  sourcesSignalsVector = []
  sourcesSignalsVector.append(
      sourcesSignalsFloat)  # Create a vector of float3DReg slices
  ...
  # Allocate model
  modelFloat = SepVector.getSepVector(elasticParamFloat.getHyper(),
                                      storage="dataFloat")
  ...
  # Outputs
  return modelFloat, dataFloat, elasticParamFloat, parObject, sourcesSignalsVector, sourcesVectorCenterGrid, sourcesVectorXGrid, sourcesVectorYGrid, sourcesVectorZGrid, sourcesVectorXZGrid, sourcesVectorXYGrid, sourcesVectorYZGrid, recVectorCenterGrid, recVectorXGrid, recVectorYGrid, recVectorZGrid, recVectorXZGrid, recVectorXYGrid, recVectorYZGrid
...
class BornElasticShotsGpu_3D(Op.Operator):
  ...
  def __init__(self, domain, range, elasticParam, paramP, sourcesSignalsVector,
    ...
    self.pyOp = pyElastic_iso_float_born_3D.BornElasticShotsGpu_3D(
        elasticParam, paramP, sourcesSignalsVector, sourcesVectorCenterGrid,
        sourcesVectorXGrid, sourcesVectorYGrid, sourcesVectorZGrid,
        sourcesVectorXZGrid, sourcesVectorXYGrid, sourcesVectorYZGrid,
        recVectorCenterGrid, recVectorXGrid, recVectorYGrid, recVectorZGrid,
        recVectorXZGrid, recVectorXYGrid, recVectorYZGrid)
    return
  ...
  def forward(self, add, model, data):
    ...
    if ("getCpp" in dir(data)):
      data = data.getCpp()
    with pyElastic_iso_float_born_3D.ostream_redirect():
      self.pyOp.forward(add, model, data)
    return
