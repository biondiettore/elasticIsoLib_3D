...
# IO object
  ...
  stop, logFile, saveObj, saveRes, saveGrad, saveModel, prefix, bufferSize, iterSampling, restartFolder, flushMemory, info = inversionUtils_3D.inversionInit(
      sys.argv)
  ...
  # Born operator
  _, _, _, _, sourcesSignalsVector, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = Elastic_iso_float_3D.BornOpInitFloat_3D(
      sys.argv)
  ...
  nonlinearElasticOp = Elastic_iso_float_3D.nonlinearFwiPropElasticShotsGpu_3D(
      modelInitConv, dataFloat, sourceFloat, parObject.param,
  ...
  BornElasticOp = Elastic_iso_float_3D.BornElasticShotsGpu_3D(
      modelInit, dataFloat, modelInitConv, parObject.param,
  ...
  if (spline == 1):
    ...
    # Parameter parsing
    _, _, zOrder, xOrder, yOrder, zSplineMesh, xSplineMesh, ySplineMesh, zDataAxis, xDataAxis, yDataAxis, nzParam, nxParam, nyParam, scaling, zTolerance, xTolerance, yTolerance, zFat, xFat, yFat = interpBSplineModule_3D.bSplineIter3dInit(
        sys.argv, zFat=0, xFat=0, yFat=0)
    splineOp = interpBSplineModule_3D.bSplineIter3d(
        modelCoarseInit, modelFineInit, zOrder, xOrder, yOrder, zSplineMesh,
        xSplineMesh, ySplineMesh, zDataAxis, xDataAxis, yDataAxis, nzParam,
        nxParam, nyParam, scaling, zTolerance, xTolerance, yTolerance, zFat,
        xFat, yFat)
